"""
Scraper for TRF5 (Tribunal Regional Federal da 5a Regiao) using Scrapling
Covers: AL, CE, PB, PE, RN e SE (nordeste)
...
class TRF5Scrapling(BaseScraper):
    """Scraper for TRF5 using Scrapling with stealth Playwright"""

    def __init__(self):
        super().__init__()
        self.tribunal_nome = "Tribunal Regional Federal da 5a Regiao"
        self.tribunal_sigla = "TRF5"
        self.base_url = "https://pje.trf5.jus.br"
        self.consulta_url = "https://pje.trf5.jus.br/pje/ConsultaPublica/listView.seam"
    ...
    def _extrair_processo(self, page, numero: str, url: str) -> Optional[ProcessoInfo]:
        """Extract process from TRF5 PJe page"""
        try:
            ...
            if '403' in page_text[:200] or 'Forbidden' in page_text[:200]:
                return ProcessoInfo(
                    ...
                    assunto="Acesso bloqueado - acesse diretamente o portal do TRF5"
                )
            ...
            cnj_match = re.search(r'(\d{7}-\d{2}\.\d{4}\.4\.05\.\d{4})', page_text)
            if cnj_match:
                processo.numero_unico = cnj_match.group(1)

            for selector in ['.processo-classe', '.classe-processual']:
            ...
            partes = []
            for el in page.css('.parte-nome, .parteNome'):
                t = el.text.strip()
            ...
            movimentacoes = []
            for el in page.css('.andamento-item, .timeline-item'):
                try:
            ...
            print(f"Erro ao extrair TRF5: {e}", file=sys.stderr)
            return None
            ...
            if page:
                ...
                if processo:
                    ...
                    resultado.processos = [ProcessoInfo(
                        ...
                        assunto="Acesse o portal do TRF5 para ver os detalhes completos"
                    )]
            else:
                resultado.erro = "Nao foi possivel acessar o portal do TRF5"

        except Exception as e:
            resultado.erro = f"Erro ao consultar TRF5: {str(e)}"
    ...
    async def buscar_por_nome(self, nome: str) -> ResultadoBusca:
        ...
        resultado.processos = [ProcessoInfo(
            ...
            assunto=f"Busca por nome no TRF5: acesse o portal para pesquisar por '{nome}'"
        )]
        return resultado
